...
void RealtimeThread::elevateToRealTimePriority(std::thread::native_handle_type thread_native_handle, bool is_main_process) {
        ...
        ret = pthread_getschedparam(thread_native_handle, &sch_policy, &sch_params);
        if (ret != 0) {
            std::cerr << "[ERROR] Failed to get thread scheduling policy and params: " << strerror(errno) << std::endl;
        }

        // Set desired scheduling policy and priority
        sch_policy = SCHED_OTHER; // SCHED_FIFO may not be allowed
        sch_params.sched_priority = 0; // Default priority

        ret = pthread_setschedparam(thread_native_handle, sch_policy, &sch_params);
        if (ret != 0) {
            std::cerr << "[ERROR] Failed to set thread scheduling policy. Error: " << strerror(errno) << std::endl;
            std::cout << "[WARNING] Adjusting thread priorities may not be supported on this device or may require special permissions." << std::endl;
        }
